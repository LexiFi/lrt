...
let attr_prop ctx =
  let prop a b =
    pexp_tuple ~loc:a.loc [ estring ~loc:a.loc a.txt; estring ~loc:b.loc b.txt ]
  in
  Attribute.declare (ppx.id ^ ".prop") ctx
    Ast_pattern.(
...
let props_of_attr attr x =
  match Attribute.get attr x with None -> [] | Some l -> l
...
type abstract = No | Auto | Name of label loc

let attr_td_abstract =
  Attribute.declare (ppx.id ^ ".abstract") Attribute.Context.type_declaration
    Ast_pattern.(
      estring __' |> single_expr_payload
      |> map1 ~f:(fun s -> Name s)
      ||| (pstr nil |> map0 ~f:Auto))
    (fun x -> x)

let abstract_of_td td =
  match Attribute.get attr_td_abstract td with Some x -> x | None -> No

let attr_ct_patch =
  Attribute.declare (ppx.id ^ ".patch") Attribute.Context.core_type
...
let lazy_value_binding ~loc txt typ expr =
  ...
  let open M in
  let pat =
    ppat_constraint (ppat_var { loc; txt }) [%type: [%t typ] Ttype.t lazy_t]
  in
...
let wrap_runtime ~loc =
  let expr = pmod_ident ~loc { txt = lid_runtime; loc } in
  pexp_open ~loc (open_infos ~loc ~expr ~override:Override)
...
let rec core_type ~rec_ ~free ({ ptyp_loc = loc; _ } as ct) : expression =
    ...
    | None, Rec l ->
        fun id args ->
          ...
          | Some l ->
              ...
              if is = should then pexp_ident ~loc id' |> force_lazy ~loc
              else raise_errorf ~loc "non-regular type recursion not supported"
          | None ->
              pexp_apply ~loc (pexp_ident ~loc id')
                (List.map (fun x -> (Nolabel, rc x)) args)
          )
  ...
  let t =
    ...
    | Ptyp_arrow (label, l, r) ->
        ...
        [%expr
          ttype_of_stype
            (DT_arrow ([%e estring ~loc lab], [%e rcs l], [%e rcs r]))]
...
let fields_of_record_labels ~rec_ ~free l =
  List.fold_left
    (fun (meta, args) ({ pld_loc = loc; _ } as x) ->
      let props = props_of_rf x and ct = core_type ~rec_ ~free x.pld_type in
...
let record_labels ~loc ~me ~free ~rec_ ~unboxed l =
  ...
  and setnode =
    ...
    and expr =
      let repr =
        if unboxed then [%expr Record_unboxed]
...
let record_labels_inline ~loc ~free ~rec_ ~name ~unboxed i l =
  ...
  let repr =
    if unboxed then [%expr Record_unboxed]
    else [%expr Record_inline [%e eint ~loc i]]
  in
  [%expr
...
let variant_constructors ~loc ~me ~free ~rec_ ~unboxed l =
      ...
      (fun ({ pcd_loc = loc; _ } as x) ->
        ...
        | Pcstr_tuple ctl ->
            ...
            let l =
              List.map
                (fun ct ->
                  core_type ~rec_ ~free ct |> fun e -> stype_of_ttype ct e)
                ctl
            in
            [%expr
              [%e estring ~loc x.pcd_name.txt],
                [%e elist ~loc props],
                C_tuple [%e elist ~loc l]]
        | Pcstr_record lbl ->
            let name = Format.sprintf "%s.%s" me.typ x.pcd_name.txt in
            let r =
              record_labels_inline ~rec_ ~free ~loc ~unboxed ~name !nconst_tag
                lbl
            in
            incr nconst_tag;
            [%expr
  ...
  and setnode =
    ...
    and expr =
      let repr =
        if unboxed then [%expr Variant_unboxed] else [%expr Variant_regular]
      in
      [%expr
...
let substitution_of_free_vars ~loc ~me ttype free =
  ...
  let expr =
    let arr =
      List.map (fun v -> [%expr stype_of_ttype [%e evar ~loc v]]) free
    in
    List.fold_left
      (fun acc v -> pexp_fun ~loc Nolabel None (pvar ~loc v) acc)
      [%expr
        ttype_of_stype
      ...
      (List.rev free)
    |> wrap_runtime ~loc
  and pat = ppat_constraint ~loc (pvar ~loc me.ttyp) typ in
  value_binding ~loc ~expr ~pat

let open_abstract ~loc ~free name =
  let f = stypes_of_free ~loc free in
  let name = estring ~loc name in
  [%expr
    let () = register_abstract_name [%e name] in
...
let str_type_decl ~loc ~path (recflag, tds) =
  ...
  let rec_ =
    ...
    | Recursive ->
        Rec
          (List.map
             (fun td -> (td.ptype_name.txt, free_vars_of_type_decl td))
             tds)
  in
  ...
  let parse (tpl, cn, lr, sn, fl, bnd) ({ ptype_loc = loc; _ } as td) =
    ...
    let cn, ttype, sn =
      ...
      | No ->
          ...
          | Ptype_variant l ->
              let unboxed = unboxed_of_td td in
              let c, t, s =
                variant_constructors ~me ~unboxed ~free ~loc ~rec_ l
              in
              (extend_let c cn, t, extend_let s sn)
          | Ptype_open -> raise_errorf ~loc "type kind not yet supported"
          )
    in
    ...
    let bnd =
      if free = [] then bnd
      else substitution_of_free_vars ~loc ~me ttyp free :: bnd
    in
    (tpl, cn, lr, sn, fl, bnd)
  in
  let tpl, createnode, lazyrec, setnode, forcelazy, bindings =
    let id x = x in
    List.fold_left parse ([], id, [], id, [], []) tds
  in
  let prepare =
    let pat, force =
      match tpl with
    ...
    let expr =
      let recflag =
        match rec_ with Nonrec _ -> Nonrecursive | _ -> Recursive
      in
      wrap_runtime ~loc
        (createnode @@ pexp_let ~loc recflag lazyrec @@ setnode @@ force)
    in
