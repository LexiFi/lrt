...
let variant_round t value = to_variant ~t value |> of_variant ~t

let string_round t value =
  to_variant ~t value
  |> Format.asprintf "%a" print_variant
  |> variant_of_string |> of_variant ~t

let round cmp t v =
  cmp v (variant_round t v) = 0 && cmp v (string_round t v) = 0
...
let%test _ =
  round b_t
    [ 0.; neg_infinity; infinity; max_float; min_float; 1.17e99; 1.17e-99; nan ]
...
let () =
  Hashtbl.add ht "a" (Some { d1 = 1; d2 = nan });
...
let%test _ =
  of_variant ~t:d_t (Record [ ("d1", Int 0); ("d2", Float 0.) ])
  = { d2 = 0.; d1 = 0 }

let%test _ =
  of_variant ~t:d_t (Record [ ("d2", Float 0.); ("d1", Int 0) ])
  = { d2 = 0.; d1 = 0 }

(* of_variant_old_name, of_variant_default *)
let%test _ = of_variant ~t:d_t (Record [ ("d", Int 0) ]) = { d2 = 1e-4; d1 = 0 }
...
let%test _ = of_variant ~t:e_t (Constructor ("A", Some (String "a"))) = A "a"

let%test _ = of_variant ~t:e_t (Constructor ("E1", Some (String "a"))) = A "a"

let%test _ =
  of_variant ~t:e_t
    (Constructor ("E3", Some (Record [ ("native", String "0") ])))
  = C { i = Nativeint.zero }
...
let f_custom = function Int i -> Some (i, string_of_int i) | _ -> None
...
let%expect_test _ =
  Format.printf "%a\n%!" Ttype.print [%t: f];
  [%expect
    {| (int * string) [@prop {of_variant_custom_uid = "0"; of_variant_custom_uid = "1"; of_variant_custom_uid = "2"}] |}]

let of_variant_string ~t s = variant_of_string s |> of_variant ~t
