...
let rec to_variant : type a. t:a Xtype.t -> a to_variant =
 fun ~t x ->
...
module Custom_of : sig
  ...
  let get uid =
    try IntMap.find (int_of_string uid) !map with
...
let rec of_variant : type a. t:a Xtype.t -> Stype.properties -> a of_variant =
 fun ~t properties v ->
  ...
  try
    ...
    | String, String x -> x
    | Char, String x ->
        if String.length x = 1 then x.[0]
        else bad_variant "single character string expected"
    ...
    | Tuple tup, Tuple l -> Builder.tuple tup (field_builder fw_packed l)
    | Record r, Record l -> Builder.record r (record_field_builder fw_packed l)
    | Sum s, Constructor (name, args) ->
        ...
        let handle_constr c =
          ...
          | Regular c, Some singleton ->
              Builder.regular_constructor c
                (field_builder fw_packed [singleton])
        ...
        in
        let next_old old (_, props) constr =
          ...
          | x, _ -> x
        in
        let rec use_first old = function
          ...
          | hd :: tl -> (
            ...
            | T2 (module A : VARIANTIZABLE_2) -> (
              try
                ...
                let TypEq.Eq = U.eq in
                A.of_variant fw_packed t1 t2 v
              with Unify.Not_unifiable -> use_first tl ) )
        in
        use_first (Hashtbl.find_all abstract_variantizers name)
    ...
    | Tuple _, _ | Record _, _ | Sum _, _ -> bad_variant "type mismatch"
      ...
      | Some (uid, reduced_props) -> (
          ...
          | Some TypEq.Eq -> (
            ...
            | exception e ->
                ...
                in
                failwith (pre ^ Printexc.to_string e) ) )
    in
    use_first properties
...
and record_field_builder : type a.
    failwith -> (string * t) list -> a Builder.t' =
 fun {failwith} lst ->
  let lref = ref lst in
      ...
      | None -> (
        ...
        | Some (default, _reduced_props) -> (
          ...
          | Variant_parser {msg; text; loc} ->
              raise
                (Variant_parser {msg= "of_variant_default: " ^ msg; text; loc})
          | Bad_type_for_variant (typ, v, msg) ->
              raise
                (Bad_type_for_variant (typ, v, "of_variant_default: " ^ msg)) )
        (* Not able to recover *)
        | None -> failwith ("missing field in variant: " ^ name) ) )
  in
  {mk}

and bad_variant : type a. a Ttype.t -> t -> failwith =
 fun t v ->
...
let of_variant_mapper ?name ~t mapper =
  let custom v =
    match mapper v with None -> None | Some v' -> Some (of_variant ~t v')
  in
  of_variant_custom ?name ~t custom
...
let rec print_variant parens ppf =
  ...
  | Tuple l ->
      pp_open_box ppf 1 ;
      pp_print_char ppf '(' ;
      Ext.List.pp_list "," (print_variant false) ppf l ;
      pp_print_char ppf ')' ;
      pp_close_box ppf ()
  | List l ->
      pp_open_box ppf 1 ;
  ...
  | Array l ->
      pp_open_box ppf 2 ;
      pp_print_string ppf "[|" ;
      Ext.Array.pp_array ';' (print_variant false) ppf l ;
      pp_print_string ppf "|]" ;
      pp_close_box ppf ()
  | Record n_v ->
      let print_field ppf (name, v) =
        pp_open_box ppf 1 ;
        pp_print_string ppf name ;
        pp_print_string ppf " =" ;
        pp_print_space ppf () ;
        print_variant false ppf v ;
        pp_close_box ppf ()
      in
      pp_open_hvbox ppf 1 ;
      pp_print_char ppf '{' ;
      Ext.List.pp_list ";" print_field ppf n_v ;
      pp_print_char ppf '}' ;
      pp_close_box ppf ()
  ...
  | Lazy v -> (
    ...
    with exn ->
      pp_may_left_paren ppf parens ;
      pp_print_string ppf "lazy(raise \"" ;
...
let () =
  Printexc.register_printer (function
    | Bad_type_for_variant (t, v, s) ->
        Some
          (Format.asprintf
             "Bad type for value: %s@.@[** Variant: %a@.@[** Type: %a@]" s
             print_variant v Stype.print t)
    | _ -> None )
  ...
  add_abstract_2
    ( module struct
      ...
      let to_variant t1 t2 ht =
        Hashtbl.fold
          (fun key value acc -> Tuple [t1 key; t2 value] :: acc)
          ht []
        |> fun x -> List x
...
let strings_of_variant v =
  let rec loop acc = function
    ...
    | String s -> s :: acc
    | Tuple l | List l -> List.fold_left loop acc l
    ...
    | Variant v | Lazy (lazy v) -> loop acc v
  in
  loop [] v
...
let format_to_out_channel ?margin oc printer arg =
  ...
  (match margin with None -> () | Some m -> pp_set_margin ppf m) ;
  printer ppf arg ; pp_print_newline ppf ()

let format_to_file ?eol_lf ?margin outfile printer arg =
  let oc =
    match eol_lf with
    | Some () -> open_out_bin outfile
    | None -> open_out outfile
  in
  try
...
let rec same_fields fields fields' =
  ...
  | [], [] -> true
  | [], _ | _, [] -> false
  | (f, _) :: fields, (f', _) :: fields' ->
      f = f' && same_fields fields fields'
...
let compact_buffer_of_variant ?dont_compress_records ?with_more_spaces v =
  ...
  let rec compact_print_variant parens = function
    ...
    | List l ->
        ...
        if compress_records then
          match l with
          | Record fields :: (_ :: _ :: _ as other_records)
            when List.for_all
                   (function
                     | Record fields' -> same_fields fields fields'
                     | _ -> false)
                   other_records ->
              print_compact_header fields ;
              iter_sep print_compact_line ';' l
          | _ -> iter_sep_compact ';' l
        else iter_sep_compact ';' l ;
        Buffer.add_char buf ']'
    ...
    | Array l ->
        ...
        let iter f =
          Array.iteri
            (fun i v ->
              f v ;
              if i < length - 1 then Buffer.add_char buf ';' )
            l
        in
        ...
        else iter (compact_print_variant false) ;
        Buffer.add_string buf "|]"
  ...
  and print_compact_line = function
    | Record l ->
        iter_sep
          (fun (_field, value) -> compact_print_variant false value)
          ',' l
    | _ -> assert false
  and iter_sep_compact sep = function
    ...
    | x :: rest ->
        compact_print_variant false x ;
        Buffer.add_char buf sep ;
        if with_more_spaces then Buffer.add_char buf ' ' ;
        iter_sep_compact sep rest
  in
  compact_print_variant false v
...
let output_compact_string_of_variant ?dont_compress_records ?with_more_spaces
    oc v =
  Buffer.clear buf ;
  compact_buffer_of_variant ?dont_compress_records ?with_more_spaces v ;
