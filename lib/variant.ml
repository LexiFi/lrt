...
module Custom_of : sig
  ...
  let get uid =
    ...
    with Not_found | Failure _ -> failwith "invalid of_variant_custom_uid"
end
...
and record_field_builder : type a. failwith -> (string * t) list -> a Builder.t'
  =
 fun { failwith } lst ->
  let lref = ref lst in
    ...
    | List l ->
        ...
        if compress_records then
          ...
          | Record fields :: (_ :: _ :: _ as other_records)
            when List.for_all
                   (function
                     | Record fields' -> same_fields fields fields' | _ -> false)
                   other_records ->
...
let output_compact_string_of_variant ?dont_compress_records ?with_more_spaces oc
    v
  =
  Buffer.clear buf;
  compact_buffer_of_variant ?dont_compress_records ?with_more_spaces v;
