...
module Types = struct
  ...
  let ttype_of_stype (type a) (s : Stype.t) : a Ttype.t = Obj.magic s
  ...
  let record_representation (l : Stype.t list) : Stype.record_repr =
    let p = Stype.equality_modulo_props (stype_of_ttype Std.float_t) in
...
module Path = struct
  ...
  let set_nth l nth x =
    ...
    in
    if nth < 0 then raise (Invalid_argument "List.nth") else f [] l nth

  let set_nth_opt l nth x =
    match set_nth l nth x with x -> Some x | exception _ -> None
