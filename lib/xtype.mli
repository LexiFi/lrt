...
(** Strip the outer properties from the type. The result is guaranteed to be
    different from {!constructor:Prop}. *)
...
module Read : sig
  ...
  val regular_constructor
    :  ('a, 'b) regular_constructor ->
    ('b, 'c) element ->
    'a ->
    'c option

  val inlined_constructor
    :  ('a, 'b) inlined_constructor ->
    ('b, 'c) element ->
    'a ->
    'c option
...
module Make : sig
  ...
  *)
  ...
  val regular_constructor : ('a, 'b) regular_constructor -> ('b t -> unit) -> 'a

  val inlined_constructor : ('a, 'b) inlined_constructor -> ('b t -> unit) -> 'a
end
