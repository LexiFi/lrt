...
let rec xtype_of_ttype : type a. a Ttype.t -> a xtype =
 fun t ->
  ...
  | DT_tuple l ->
      cast_xtype (Tuple { t_flds = fields l; t_len = List.length l })
...
module Assembler = struct
  ...
  let init'
      :  int -> int -> 'a record_field list -> 'b asm -> (string * 'b) list ->
      Obj.t
    =
   fun tag arity flds asm ->
    let farr = lazy (fun_arr_of_flds' arity flds asm) in
...
module Make = struct
  ...
  let regular_constructor
      : type a b. (a, b) regular_constructor -> (b t -> unit) -> a
    =
   fun c f -> Builder.regular_constructor c (builder f c.rc_len)

  let inlined_constructor
      : type a b. (a, b) inlined_constructor -> (b t -> unit) -> a
    =
...
module Read = struct
  ...
  let map_sum : type a b c. a sum -> b mapf -> c mapf' -> a -> (b, c) mapped_sum
    =
   fun sum mapf mapf' ->
    let aux =
      lazy
  ...
  | (_, meta) :: tl ->
      let t =
        ...
        | Field { field_name }, Record r ->
            r.r_lookup field_name |> assert_some |> ( function
            | _, Field f -> cast f.typ )
        | Tuple { nth; _ }, Tuple t ->
        ...
        | Constructor { name; arg }, Sum s ->
            ...
            | Inline { field_name }, Inlined c ->
                c.ic_lookup field_name |> assert_some |> ( function
                | _, Field f -> cast f.typ )
