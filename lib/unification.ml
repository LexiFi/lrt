...
let t0 (type t) (t: t Ttype.t) : (module T0 with type t = t) =
  (module struct type nonrec t = t [@@deriving t] end)
...
let unify ~modulo_props nfree t1 t2 =
  ...
  and equal = if modulo_props then
  ...
  and s2 = Ttype.to_stype t2
  in
  let rec unify s1 s2 =
    let set v s =
      match Array.get subs v with
      | None -> subs.(v) <- Some s
      | Some s' -> if not (equal s s') then raise Not_unifiable
    in
    match (s1, s2: Stype.t * Stype.t) with
    | _, DT_var _ ->
      raise (Invalid_argument "unify: free variable in ttype")
...
let ttype: type a. Stype.t option -> a Ttype.t = function
  | None -> Obj.magic Std.unit_t (* Unification succeeded, but type variable was
                                    not used. *)
