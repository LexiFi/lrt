...
type ctx = { to_json_field: string -> string; matcher: Matcher.t }
  ...
  and conv_record : type a. a record -> a conv =
   fun r ->
    ...
    and of_json =
      ...
      function Object l -> asm (fix_names l) | _ -> failwith "object expected"
    in
    { to_json; of_json }
            ...
            let exponent () =
              ...
              if State.eof t then fail ()
                ...
                if index0 = State.index t then fail ()
                  ...
                  succ
                    (F
                       (float_of_substring (State.buf t) i0
                          (State.index t - i0)))
            in
...
module Access = struct
  ...
  let mk_get_field of_value field obj =
    ...
    try of_value v
    with exn -> failwithf "%s: %s" field (Printexc.to_string exn)
