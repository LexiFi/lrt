...
let cast ~loc name e =
  let_in [Vb.mk (pvar name) [%expr
            (* Format.(fprintf err_formatter "%a\n%!" Types.print_stype [%e evar name]); *)
            Obj.magic [%e evar name]]] e

(* Construct record ttypes *)
let str_of_record_labels ?inline ~loc ~opt ~name ~mangled l =
  ...
  | None ->
    [%expr make_record ~name:[%e name] []
        (fun [%p pvar mangled] -> [%e cast ~loc mangled ll]) |> Obj.magic ]
  | Some i ->
    [%expr make_record ~name:[%e name] ~inline:[%e int i] []
        (fun _ -> [%e ll]) |> Obj.magic ]

(* Construct variant ttypes *)
let str_of_variant_constructors ~loc ~opt ~name ~mangled l =
  ...
  let ll = List.rev_map (fun {pcd_loc = loc; pcd_name; pcd_args; _ } ->
      ...
      | Pcstr_record lbl ->
        let r =
          str_of_record_labels ~inline:!nconst_tag ~mangled
...
let str_of_type_decl ~options ~path ({ ptype_loc = loc ; _} as td) =
  ...
  let t = match td.ptype_kind with
    ...
    | Ptype_variant l ->
      str_of_variant_constructors ~loc ~opt ~name ~mangled l
    | Ptype_record l -> str_of_record_labels ~loc ~opt ~name ~mangled l
    | Ptype_open ->
